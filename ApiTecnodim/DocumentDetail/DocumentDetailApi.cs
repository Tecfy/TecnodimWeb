...
namespace ApiTecnodim
{
    public partial class DocumentDetailApi
    {
        public DocumentDetailOut GetDocumentDetail(DocumentDetailIn documentDetailIn)
        {
            try
            {
                var client = new RestClient(WebConfigurationManager.AppSettings["ApiTecnodim.URL"].ToString() + string.Format(WebConfigurationManager.AppSettings["ApiTecnodim.DocumentDetail.GetDocumentDetail"].ToString(), documentDetailIn.externalId.ToString()));
