...
public static class HelperImage
{
    ...
    /// <param name="typeImage">Image Type </param>
    /// <param name="background">Image background color when it has left over</param>originalWidth
    public static byte[] Resize(Image image, int newSize, TypeSize typeSize, int? otherSize, int quality, bool squareImage, TypeImage typeImage, Color background)
    {
        int originalWidth = image.Width;
        int originalHeight = image.Height;
        ...
        {
            imageResize.Save(stream, imageCodecInfos[(int)typeImage], encoderParameters);
            image.Dispose();
